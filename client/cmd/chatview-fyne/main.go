...
import (
	...
	"chatview/internal/platform"
	"chatview/internal/rpcclient"
	"chatview/internal/storage"
	chatui "chatview/internal/ui"
)
...
func warnIfLikelyBlurry() {
	...
	fmt.Fprintln(os.Stderr, "warning: running on Wayland without native Wayland build tag; use `go run -tags wayland ./cmd/chatview-fyne` or `./scripts/run-fyne` if the window looks blurry")
}
